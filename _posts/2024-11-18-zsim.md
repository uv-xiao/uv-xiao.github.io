...
`struct BblInfo` stores BBL information including size, #ins, uops, and **decoder cycles**. The last two is used for OOO core model only. In `BblInfo* Decoder::decodeBbl(BBL bbl, bool oooDecoding)` from `decoder.cpp`, Macro-op fusion is tried (decoder.cpp:1332). It models two steps of OOO decoder: pre-decode and decode. The docoding cycles are stored in `bblInfo->oooBbl[0].uop[i].decCycle`. The decoder is shared by all core models.
