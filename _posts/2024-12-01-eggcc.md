...
```
Writing original build to /home/uvxiao/repos/eggcc/dag_in_context/temp/egglog_buildonly.egg
Writing egglog test to /home/uvxiao/repos/eggcc/dag_in_context/temp/egglog_test.egg
```
both of which contains thousands of lines.
...
> The Slidemode acts very strange; the produced e-graph differs from the complete run. NOT RECOMMENDED!
{: .block-danger}

For example, two e-graphs (before and after schedule-runs) are produced for the `peepholes::arith_rewrites` test:

<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid path="assets/img/in_posts/2024-12-01-eggcc-egglog_buildonly.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
<div class="caption">
  e-graphs before schedule-runs
</div>

<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid path="assets/img/in_posts/2024-12-01-eggcc-egglog_test.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
<div class="caption">
  e-graphs after schedule-runs
</div>
...
2. **TODO** 
   1. mimic `PrettyPrinter`, `build_program` and `print_with_intermediate_helper` to maintain a `TreeToEgglog` and `term_cache` (from egraph `Term`s to variable names) for test egglog program.
   2. support `extract` a given variable. (maybe extract the related term?); refer to `PrettyPrinter::from_string` and `dag_in_context::greedy_dag_extractor::test_dag_extract`
